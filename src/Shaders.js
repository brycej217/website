...
function ballUpdate(delta) {
  ...
  const sim = Array.from({ length: count }, () => ({
    pos: new THREE.Vector3().random().subScalar(0.5), // map [0, 1) -> [-0.5, 0.5)
    velocity: new THREE.Vector3().random().subScalar(0.5).multiplyScalar(speed),
  }))
  ...
  return (mesh, delta) => {
    ...
    for (let i = 0; i < count; ++i) {
      ...
      // bounds checking
      /*
      const d = new THREE.Vector3().subVectors(center, pos)
      const dist = d.length()
      if (dist > radius) {
        d.normalize()
        velocity.copy(d).multiplyScalar(speed)
      }*/
